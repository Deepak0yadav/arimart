    ...
    <nav className="md:hidden fixed top-0 left-0 right-0 z-50 w-full border-b bg-green-50 shadow-sm border-green-100">
      <div className="flex items-center justify-between px-3 py-2">
        ...
        <div className="flex items-center gap-1">
          ...
          <Link
            href="/group-buying"
            className="flex items-center gap-1 text-gray-800 font-medium hover:text-gray-600 transition-colors p-1 rounded-full hover:bg-gray-100">
      ...
      {navitems.length > 0 && (
        <div className="flex overflow-x-auto gap-3 px-3 py-2 scrollbar-hide bg-green-50">
          {navitems.map((item) => (
            <Link
              key={item.name}
